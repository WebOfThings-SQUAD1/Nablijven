...
Mario.MapState.prototype.GenerateLevel = function() {
    ...
    var n0 = new Mario.ImprovedNoise(1);
    var n1 = new Mario.ImprovedNoise(2);
    var dec = new Mario.ImprovedNoise(3);
    ...
    for (i = 0; i < 100 && t < 12; i++) {
        x = ((pseed(1+i) * (((width - 1) / 3) | 0)) | 0) * 3 + 2;
        y = ((pseed(1+i/1000) * (((height - 1) / 3) | 0)) | 0) * 3 + 1;
...
Mario.MapState.prototype.Travel = function(x, y, dir, depth) {
    ...
    if (this.Level[x][y] === Mario.MapTile.Level) {
        if (this.Data[x][y] > 0) {
            if (this.LevelId !== 0 && ((pseed(x) * 4) | 0) === 0) {
                this.Data[x][y] = -3;
            } else {
...
Mario.MapState.prototype.RenderStatic = function() {
    ...
    for (x = 0; x < 20; x++) {
        ...
        for (y = 0; y < 15; y++) {
            ...
            if ((type==-2||type==0)&&this.Level[x][y] === Mario.MapTile.Level) {


                //this.MapContext.drawImage(image, type * 16, 6 * 16, 16, 16, x * 16, y * 16, 16, 16);
                //continue
                // if (type === -2){
            ...
            } else if (this.Level[x][y] === Mario.MapTile.Road||this.Level[x][y]===Mario.MapTile.Level) {
...
Mario.MapState.prototype.TryWalking = function(xd, yd) {
    ...
    if (this.Level[xt][yt] === Mario.MapTile.Road || this.Level[xt][yt] === Mario.MapTile.Level) {
        // if (false&&this.Level[xt][yt] === Mario.MapTile.Road) {
        //     if ((false&&this.Data[xt][yt] !== 0) && (this.Data[x][y] !== 0 && this.Data[x][y] > -10)) {
        //         return;
        //     }
        // }
...
Mario.MapState.prototype.CheckForChange = function(context) {
    //comm: === x means starting difficoulty. goes until 8
    if (this.WorldNumber === 8) {
        context.ChangeState(new Mario.WinState());
    }
    if (this.EnterLevel) {
        //context.ChangeState(new Mario.LevelState(this.LevelDifficulty, this.LevelType));
        context.ChangeState(new Mario.LevelState(2, this.LevelType));
