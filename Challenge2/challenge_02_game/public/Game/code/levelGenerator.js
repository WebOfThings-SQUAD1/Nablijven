...
Mario.LevelGenerator.prototype = {
    ...
    BuildZone: function(level, x, maxLength) {
        var t = (pseed() * this.TotalOdds) | 0, type = 0, i = 0;
        for (i = 0; i < this.Odds.length; i++) {
            if (this.Odds[i] <= t) {
                type = i;
            }
    ...
    BuildJump: function(level, xo, maxLength) {
        var js = ((pseed() * 4) | 0) + 2, jl = ((pseed() * 2) | 0) + 2, length = js * 2 + jl, x = 0, y = 0,
            hasStairs = ((pseed() * 3) | 0) === 0, floor = this.Height - 1 - ((pseed() * 4) | 0);
    ...
    BuildCannons: function(level, xo, maxLength) {
        var length = ((pseed() * 10) | 0) + 2, floor = this.Height - 1 - (pseed() * 4) | 0,
            xCannon = xo + 1 + (pseed() * 4) | 0, x = 0, y = 0, cannonHeight = 0;
        ...
        for (x = xo; x < xo + length; x++) {
            ...
            cannonHeight = floor - ((pseed() * 4) | 0) - 1;
    ...
    BuildHillStraight: function(level, xo, maxLength) {
        var length = ((pseed() * 10) | 0) + 10, floor = this.Height - 1 - (pseed() * 4) | 0,
    ...
    AddEnemyLine: function(level, x0, x1, y) {
        var x = 0, type = 0;
        for (x = x0; x < x1; x++) {
            if (((pseed() * 35) | 0) < this.Difficulty + 1) {
                type = (pseed() * 4) | 0;
                ...
                level.SetSpriteTemplate(x, y, new Mario.SpriteTemplate(type, ((pseed() * 35) | 0) < this.Difficulty));
            }
        }
    },
    
    BuildTubes: function(level, xo, maxLength) {
        var length = ((pseed() * 10) | 0) + 5, floor = this.Height - 1 - (pseed() * 4) | 0,
            xTube = xo + 1 + (pseed() * 4) | 0, tubeHeight = floor - ((pseed() * 2) | 0) - 2,
            x = 0, y = 0, xPic = 0;
        ...
        for (x = xo; x < xo + length; x++) {
            if (x > xTube + 1) {
                xTube += 3 + ((pseed() * 4) | 0);
                tubeHeight = floor - ((pseed() * 2) | 0) - 2;
            }
            if (xTube >= xo + length - 2) {
                xTube += 10;
            }
            
            if (x === xTube && ((pseed() * 11) | 0) < this.Difficulty + 1) {
    ...
    BuildStraight: function(level, xo, maxLength, safe) {
        var length = ((pseed() * 10) | 0) + 2, floor = this.Height - 1 - ((pseed() * 4) | 0), x = 0, y = 0;
    ...
    Decorate: function(level, x0, x1, floor) {
        ...
        var rocks = true, s = (pseed() * 4) | 0, e = (pseed() * 4) | 0, x = 0;
